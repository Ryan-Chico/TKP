...
export default function AssetListScreen({ route, navigation }) {
    ...
    const sections = useMemo(() => {
        // MODIFIED: Filter out any assets that are marked as not visible.
        const visibleAssets = assets.filter(asset => asset.isVisible !== false);

        const building = visibleAssets.filter(asset => asset.category !== 'Relics/Artifacts');
        const relics = visibleAssets.filter(asset => asset.category === 'Relics/Artifacts');
