...
// Import hooks and firebase utils
...
const slides = [
    ...
    },
    {
        ...
        animation: { uri: 'https://lottie.host/a19a51aa-a3a3-42ba-bc50-868c82229f6a/044lm95GZu.lottie' },
    },
    {
        key: '5',
...
const IntroductionScreen = () => {
    ...
    const { user, isGuest } = useUser(); // Get user from context
    const [activeIndex, setActiveIndex] = useState(0);
    const scrollX = useRef(new Animated.Value(0)).current;
    const scrollViewRef = useRef<ScrollView>(null);

    // This function updates Firestore and then navigates
    const markIntroAsViewedAndNavigate = async () => {
        // Only update for registered users
        if (user && !isGuest) {
            try {
                const userDocRef = doc(db, 'users', user.uid);
                await updateDoc(userDocRef, { hasViewedIntro: true });
            } catch (error) {
    ...
    return (
        <SafeAreaView style={styles.container}>
            <TouchableOpacity style={styles.skipButton} onPress={markIntroAsViewedAndNavigate}>
            ...
            <View style={styles.bottomContainer}>
                ...
                {activeIndex === slides.length - 1 ? (
                    <TouchableOpacity style={styles.button} onPress={markIntroAsViewedAndNavigate}>
